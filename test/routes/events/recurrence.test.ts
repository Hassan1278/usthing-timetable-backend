...
test("occurrence reminder settings resolve defaults and preserve parent preferences", async () => {
  const series = await create({ ...input, allowConflicts: true });
  const key = "2026-10-12T10:00:00Z";
  const updated = await exception(series.id, key, 1, "PATCH", {
    emailNotifications: { enabled: true },
  });
  assert.equal(updated.revision, 2);
  assert.deepStrictEqual(updated.emailNotifications, { enabled: false });
  assert.deepStrictEqual((await list()).items[0].emailNotifications, {
    enabled: true,
    minutesBefore: [1440, 120],
  });
  await exception(series.id, key, 2, "PATCH", {
  ...
  await exception(series.id, key, 3, "PATCH", {
